<template>
  <v-list dense nav>
    <div v-for="(item, i) in items" :key="i">
      ...
      <v-list-group v-if="item.subItems && checkPermissionForSubItems(item.subItems)" :prepend-icon="item.icon" color="grey darken-4">
    ...
    return {
      ...
      items: [
        ...
        { title: "खाताहरु", icon: "mdi-account-circle", route: "/users" , subItems:[
            { title: "प्रयोगकर्ताहरू", icon: "mdi-account-circle", route: "/users" , check: "can_browse_users"},
  ...
  methods: {
    ...
    checkPermissionForSubItems(subItems){
      ...
      subItems.forEach(function(item){
          if(!tempthis.userPermissions.includes(item.check)){
            return true;
          }
      });
      return false;
