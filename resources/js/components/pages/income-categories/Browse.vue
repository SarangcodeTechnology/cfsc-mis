                            ...
                            <v-row>
                                <v-col cols="3">
                                    <div class="d-flex align-content-center">
                                        <h5 class="mb-0 align-self-center">आम्दानी बर्गिकरणहरु</h5>
...
export default {
    ...
    computed: {
        ...mapState({incomeCategories: (state) => state.webservice.incomeCategories}),
    },
    methods: {
        getDataFromApi() {
            const tempthis = this;
            ...
            this.$store.dispatch("getIncomeCategories", {}).then(function (response) {
                tempthis.loading = false;
            });
        },
        goToEditPage() {
            this.$store.dispatch("setIncomeCategoriesEditData", {
                title: ""
            });
        },
        editData(item) {
            this.$store.dispatch("setIncomeCategoriesEditData", item)
        },
        confirm(item) {
            const tempthis = this;
        ...
        deleteData(item) {
            let tempthis = this;
            this.$store.dispatch("deleteIncomeCategories", item).then(function (response) {
